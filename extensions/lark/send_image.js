...
const IMAGE_PATH = "/tmp/baidu_new.png";
...
async function sendImage() {
  try {
    // 步骤 1: 上传图片
    console.log("步骤 1: 上传图片到 Lark...");
    const imageStream = fs.createReadStream(IMAGE_PATH);
    const stats = fs.statSync(IMAGE_PATH);
    console.log("图片大小:", stats.size, "字节");
    ...
    console.log("上传响应完整对象:", uploadResponse);
    console.log("上传响应 code:", uploadResponse.code);

    // Lark 图片上传可能没有 code 字段，直接检查 image_key
